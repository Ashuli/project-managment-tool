<template>
  <div class="w-full h-full py-5 px-8">
    <div class="flex items-center text-textDarkest">
      <div class="text-xl">Document Type</div>
...
export default {
  ...
  computed: {
    ...mapGetters('mainStore', ['error', 'currentDocumentTypes']),
    isValidDTO() {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      const saved = await this.save({
        url: this.editMode ? `document-types/${this.currentDocumentTypes.id}` : 'document-types',
  ...
  created() {
    if (this.currentDocumentTypes && Object.keys(this.currentDocumentTypes).length) {
      this.editMode = true
      this.name = this.currentDocumentTypes.name
      this.description = this.currentDocumentTypes.description
      this.status = this.currentDocumentTypes.status
    }
  }
}
</script>
