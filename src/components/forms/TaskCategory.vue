<template>
  <div class="w-full h-full py-5 px-8">
    <div class="flex items-center text-textDarkest">
      <div class="text-xl">Task Category</div>
...
export default {
  ...
  computed: {
    ...mapGetters('mainStore', ['error', 'currentTaskCategory']),
    isValidDTO() {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      const saved = await this.save({
        url: this.editMode ? `task-categories/${this.currentTaskCategory.id}` : 'task-categories',
   ...
   created() {
    if (this.currentTaskCategory && Object.keys(this.currentTaskCategory).length) {
      this.editMode = true
      this.name = this.currentTaskCategory.name
      this.order = this.currentTaskCategory.order
      this.description = this.currentTaskCategory.description
      this.status = this.currentTaskCategory.status
    }
  }
}
